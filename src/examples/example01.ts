...
class LightOn extends LightMachine {
    switch(): void { this.transition(LightOff); }
    getReport(): string { return `the light is on and it's color is ${this.ctx.color}`}
}

@ihsm.initialState
class LightOff extends LightMachine {
    _entry() {}
    switch(): void { this.transition(LightOn); }
    getReport(): string { return `the light is off and it's color is ${this.ctx.color}`}
}

async function main() {
    try {
        ihsm.configureLogLevel(ihsm.LogLevel.TRACE);
